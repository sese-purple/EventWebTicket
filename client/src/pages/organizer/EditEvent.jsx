...
const EditEvent = () => {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        const response = await api.get(`/events/organizer/${eventId}`);
        ...
        if (event.banner_image) {
          setBannerImagePreview(event.banner_image);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: error.response?.data?.message || error.message || 'Failed to update event'
      });
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="font-sans bg-white min-h-screen flex flex-col">
        ...
        <main className="flex-grow flex items-center justify-center">
          <p className="text-[20px] font-bold text-[#656565]">Loading event...</p>
        </main>
        <Footer />
      </div>
    );
        ...
        <section className="h-auto p-4 md:p-[30px] md:py-0 md:h-[167px] bg-gradient-to-r from-[#2563eb] via-[#1d4eb8] to-[#153885] flex items-center">
          <div className="max-w-[1440px] mx-auto w-full px-2 md:px-12">
            <div className="flex items-center gap-2 md:gap-4">
              ...
              <h2 className="text-lg md:text-[40px] font-bold text-white">Edit Event</h2>
            </div>
          </div>
        </section>
